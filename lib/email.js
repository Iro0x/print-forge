    ...
    <div style="font-family:'Helvetica Neue',Arial,sans-serif;background:#0a0a0a;padding:40px 20px;min-height:100vh">
      <div style="max-width:560px;margin:0 auto;background:#111;border:1px solid #222;border-top:3px solid #ff4500;border-radius:2px;overflow:hidden">
        <div style="padding:28px 32px;border-bottom:1px solid #222">
          <span style="font-family:'Arial Black',Arial,sans-serif;font-size:22px;font-weight:900;letter-spacing:3px;color:#ff4500">PRINT</span><span style="font-family:'Arial Black',Arial,sans-serif;font-size:22px;font-weight:900;letter-spacing:3px;color:#f0ece4">-FORGE</span>
        </div>
...
function trackBox(id, baseUrl) {
  return `
    <div style="background:#0a0a0a;border:1px solid #222;border-left:3px solid #ff4500;border-radius:2px;padding:16px 20px;margin:20px 0">
      <p style="font-family:'Courier New',monospace;font-size:11px;color:#555;margin:0 0 8px;text-transform:uppercase;letter-spacing:2px">ID zamówienia (do śledzenia)</p>
      <p style="font-family:'Courier New',monospace;font-size:13px;color:#f0ece4;margin:0 0 12px;word-break:break-all">${id}</p>
      <a href="${baseUrl}/sledzenie?id=${id}" style="font-family:'Courier New',monospace;font-size:11px;color:#ff4500;text-decoration:none;text-transform:uppercase;letter-spacing:1px">Sprawdź status zamówienia →</a>
    </div>
  `
}
...
export async function sendCustomOrderConfirmation({ customerName, customerEmail, orderId }) {
  await send({
    ...
    html: base(`
      ...
      ${p('Przejrzymy Twój plik i odezwiemy się z <strong style="color:#f0ece4">wyceną w ciągu kilku godzin roboczych</strong>.')}
      ${p('Na razie nic więcej nie musisz robić.')}
      ${trackBox(orderId, process.env.NEXT_PUBLIC_BASE_URL)}
    `),
  })
}
...
export async function sendCustomOrderPaidAdminNotification({ orderId, customerName, customerEmail, fileName, totalAmount }) {
  if (!process.env.ADMIN_EMAIL) return
  await send({
    to: process.env.ADMIN_EMAIL,
    subject: `💳 Opłacono zamówienie #${orderId.slice(0, 8).toUpperCase()} — do druku!`,
    html: base(`
      ${h('Zamówienie opłacone — czas drukować! 🖨️')}
      ${oid(orderId)}
      ${p(`<strong style="color:#f0ece4">Klient:</strong> ${customerName} (${customerEmail})`)}
      ${p(`<strong style="color:#f0ece4">Plik:</strong> ${fileName || 'brak pliku'}`)}
      ${priceBox(totalAmount)}
      ${btn('Otwórz zamówienie →', `${process.env.NEXT_PUBLIC_BASE_URL}/admin`)}
    `),
  })
}
...
const STATUS_EMAILS = {
  ...
  printing: ({ name, id }) => ({
    ...
    html: base(`
      ...
      ${trackBox(id, process.env.NEXT_PUBLIC_BASE_URL)}
    `),
  }),
